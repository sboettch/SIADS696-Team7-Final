...
OUT = ROOT / "notebooks" / "Final_Report_Reproducible_Visualizations.ipynb"
...
cells = [
    md(
        ...
        It is intended to make the report figures auditable: every plot is produced from project files under `data/` or `outputs/`, then saved to `outputs/reproducible_report_figures/`.
        """
    ),
    code(
        """
        ...
        FIG_DIR = ROOT / "outputs" / "reproducible_report_figures"
        FIG_DIR.mkdir(parents=True, exist_ok=True)

        RUBRIC = ROOT / "outputs" / "rubric_completion"
        PRIMARY = ROOT / "outputs" / "primary_notebook"
    ...
    code(
        ...
        axes[0].set_title("DBSCAN sensitivity search")
        ...
        axes[1].set_title("K-Means k sensitivity search")
        ...
        manifest = pd.DataFrame(
            {
                ...
                "source": "Generated by notebooks/Final_Report_Reproducible_Visualizations.ipynb from project data/outputs",
            }
        )
